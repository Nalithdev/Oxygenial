              ...
              conversations.map((conv) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left px-4 py-3 border-b border-slate-50 hover:bg-slate-50 transition-colors ${selectedId === conv.id ? "bg-blue-50 border-l-2 border-l-blue-600" : ""}`}
                  >
                    <div className="flex items-center gap-2 mb-1">
                      <Building2 className="w-3.5 h-3.5 text-slate-400 shrink-0" />
                      <p className="font-medium text-slate-900 text-sm truncate">
                        {conv.medicalCompany?.name}
                      </p>
                    </div>
                    {last && (
                  ...
                  messagesQuery.data?.map((msg) => {
                    ...
                    return (
                      <div key={msg.id} className={`flex ${isMe ? "justify-end" : "justify-start"}`}>
                        <div className={`max-w-[70%] px-4 py-2 rounded-2xl text-sm ${isMe ? "bg-blue-600 text-white rounded-br-sm" : "bg-slate-100 text-slate-900 rounded-bl-sm"}`}>
                          <p>{msg.content}</p>
